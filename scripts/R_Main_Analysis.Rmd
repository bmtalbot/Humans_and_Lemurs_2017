...
# Version of Code
This code is maintained by Brooke M. Talbot. This is Version 2.0 updated last on 7/9/2024
...
## Supplemental Data 2 and 3 - Metaphlan 4 Results 
```{r}


## Metaphlan analysis that pulls in the relative abundances at the Kingdom level and all of the unclassified reads (supplemental 3A). 

all_profiled_k <- read.table("Supplemental_3.txt", header = T, sep = "\t")

## Kingdom Counts

bug_dat_4_k <- all_profiled_k
colnames(bug_dat_4_k)[1] <- "cladeName"
colnames(bug_dat_4_k)[2] <- "cladeTaxa"
names(bug_dat_4_k) = gsub(pattern = "new_", replacement = "", x = names(bug_dat_4_k))
names(bug_dat_4_k) = gsub(pattern = "_.*", replacement = "", x = names(bug_dat_4_k))
bug_dat_4_k <- bug_dat_4_k %>% select(-cladeTaxa)


king_dat <- bug_dat_4_k %>% 
...
## Creates a table that is the relative abundances for each of the samples when only considering bacterial relative abundance (Supplemental 3B) 
all_profiled <- read.table("Supplemental_2.txt", header = T, sep = "\t")
...
abundance_table <- abundance_table %>% filter(code3 %in% refgenes_AMR$RefSeq.nucleotide) %>%
  mutate(Template_Coverage_2 = as.numeric(gsub("[^0-9.-]", "", Template_Coverage))) %>% mutate(Query_Identity_2 = as.numeric(gsub("[^0-9.-]", "", Query_Identity))) %>%
mutate(readCount = as.numeric(readCount), .before = readCount) %>%
  ## Filtering step for identity and hit matching
  filter(Query_Identity_2 >= 90) %>%
  filter(Template_Coverage_2 >= 60) %>%
  filter(readCount >= 3)
...
table4 <- read.table("~/Supplemental_4.csv", header = FALSE, comment.char = "$", sep =',', fill = TRUE)
...
## Figure 1 Context of Read Taxa Classification
```{r}

## Total reads
Panel1 <- total_reads %>% dplyr::select(Sample, FastQC_mqc.generalstats.fastqc.total_sequences) %>% unique() %>%
  mutate(Source = case_when(grepl("HUM",Sample)~ "Human", TRUE ~ "Lemur")) %>% ggplot(aes(x = Sample , y = FastQC_mqc.generalstats.fastqc.total_sequences, group = Source )) + 
  geom_bar(stat = "identity") + 
  scale_fill_brewer(palette = "Paired") + 
  labs(y = "Total number of reads", fill = "Taxa") + 
...
moments::skewness(total_reads$FastQC_mqc.generalstats.fastqc.total_sequences %>% unique()) ## slightly skewed to the left, value is 0.69
moments::kurtosis(total_reads$FastQC_mqc.generalstats.fastqc.total_sequences %>% unique()) ## pretty close to 3, 3.669 indicating a fairly even thickness similar to normal distribution

shapiro.test(total_reads$FastQC_mqc.generalstats.fastqc.total_sequences %>% unique())
## significant results indicate the distribution is not normally distributed

xyz <- total_reads %>% unique() %>% mutate(Source = case_when(grepl("HUM",Sample)~ "Human", TRUE ~ "Lemur")) 

wilcox.test(FastQC_mqc.generalstats.fastqc.total_sequences ~ Source, data = xyz)
## p-value less than 0.05 suggesting there is a significant difference in the number of total reads between the two sample sets

## distribution
Panel_dist <- total_reads %>% dplyr::select(Sample, FastQC_mqc.generalstats.fastqc.total_sequences) %>% unique() %>%
  mutate(Source = case_when(grepl("HUM",Sample)~ "Human", TRUE ~ "Lemur")) %>% ggplot(aes(x =  FastQC_mqc.generalstats.fastqc.total_sequences, color = Source, fill = Source)) + 
...
Panel_TS <- total_reads %>% dplyr::select(Sample, FastQC_mqc.generalstats.fastqc.total_sequences) %>% unique() %>%
  mutate(Source = case_when(grepl("HUM",Sample)~ "Human", TRUE ~ "Lemur")) %>% ggplot(aes(x = Source , y = FastQC_mqc.generalstats.fastqc.total_sequences, fill = Source )) + 
  geom_boxplot() + 
  labs(y = "Total number of reads", fill = "Source") + 
scale_fill_manual(values = c("#3c0d94","#eba21a")) + 
  geom_signif(
...
shapiro.test(total_reads$FastQC_mqc.generalstats.fastqc.avg_sequence_length %>% unique())
## significant results indicate the distribution is not normally distributed
wilcox.test(FastQC_mqc.generalstats.fastqc.avg_sequence_length ~ Source, data = xyz)
## significant difference between the average sequence length of the reads, with 
xyz %>% group_by(Source) %>% summarize (m = median(as.numeric(FastQC_mqc.generalstats.fastqc.avg_sequence_length)), max = max(as.numeric(FastQC_mqc.generalstats.fastqc.avg_sequence_length)), min = min(as.numeric(FastQC_mqc.generalstats.fastqc.avg_sequence_length)))


library(scales)
Panel_AS <- total_reads %>% dplyr::select(Sample, FastQC_mqc.generalstats.fastqc.avg_sequence_length) %>% unique() %>%
  mutate(Source = case_when(grepl("HUM",Sample)~ "Human", TRUE ~ "Lemur")) %>% ggplot(aes(x = Source , y = FastQC_mqc.generalstats.fastqc.avg_sequence_length, fill = Source )) + 
  geom_boxplot() + 
  labs(y = "Average Sequence Length", fill = "Source") + 
scale_fill_manual(values = c("#3c0d94","#eba21a")) + 
  geom_signif(
   comparisons = list(c("Human", "Lemur")),
    map_signif_level = TRUE) + 
...
Panel2 <- king_dat_T %>% pivot_longer(2:5, names_to = "kingdom", values_to = "abundance") %>% ggplot(aes(x = SampleID , y = abundance, group = Source, fill = kingdom)) + 
  geom_bar(stat = "identity") +
  scale_fill_manual(values = c("#CF8400","#75AEFF","#FFC96A","#535963"), labels = c("Archaea","Bacteria","Eukaryota","Unclassified")) + 
  labs(y = "Relative Abundance (Percent)", fill = "Taxa") + 
  facet_grid(. ~ Source,  scales = "free", space = "free") +  
  theme(panel.background = element_blank(), 
        axis.text.x = element_blank(),
        axis.title.y = element_text(size = 11),
...
top <- cowplot::plot_grid(Panel_TS, Panel_AS, labels = c("a","b"))
bottom <- cowplot::plot_grid(Panel2, labels = "c")
Figure1resub <- cowplot::plot_grid(nrow = 2, ncol = 1, top, bottom, label_size = 8)


ggsave("Fig1_new_resub_dpi_300.png", Figure1resub, height = 20 , width = 25, dpi = 300, units = "cm")

Figure1resub

ggsave("Fig1_new_resub_dpi_300.pdf", Figure1resub, height = 8 , width = 12, dpi = 300, units = "cm")
...
## Figure 2 - Counts and Diversity Metrics of bacterial species

### Phyla count
```{r}


fig2A_bact <- phyla_dat_T %>%
  ...
  dplyr::select(-p__Candidatus_Saccharibacteria,-p__Deferribacteres,-p__Verrucomicrobia, -p__Bacteria_unclassified) %>%
  gather("phyla", "abundance",3:13) %>%
  mutate(phyla_new = gsub("p__","",phyla)) %>%
  mutate(phyla_new = gsub("_"," ",phyla_new ))

## Order the Legend
fig2A_bact$phyla_new <- factor(fig2A_bact$phyla_new, levels=c('Bacteroidetes','Firmicutes','Proteobacteria','Actinobacteria','Tenericutes','Spirochaetes','Candidatus Melainabacteria','Elusimicrobia','Lentisphaerae','Fusobacteria','Other Classified Bacteria','Other Phyla'))

## Panel1 plot 
Fig2A_bact <- fig2A_bact %>% ggplot(aes(x = SampleID , y = abundance, group = Source, fill = phyla_new)) + 
  geom_bar(stat = "identity") +
  scale_fill_brewer(palette = "Paired") + 
  ...
  theme(panel.background = element_blank(), 
        ...
        axis.ticks.x = element_blank(),
        legend.position = "bottom",
        legend.box = "horizontal", legend.margin=unit(-2,"cm"),
...
shannon_human <- spec_data_t2 %>% filter(grepl("HUM",SAMPLE_ID)) 
rownames(shannon_human) <- shannon_human[,1]

human_SD <- diversity(shannon_human[,2:length(shannon_human)])
...
shannon_lemur <- spec_data_t2 %>% filter(grepl("LMR",SAMPLE_ID)) 
rownames(shannon_lemur) <- shannon_lemur[,1]

lemur_SD <- diversity(shannon_lemur[,2:length(shannon_lemur)])
...
species_SI <- rownames_to_column(species_SI, var = "SAMPLE_ID")

## Both are significant
t.test(as.numeric(Shannon) ~ Source, species_SI)
wilcox.test(as.numeric(Shannon) ~ Source, species_SI)

fig2C <- ggplot(species_SI, aes(x = Source, y = as.numeric(Shannon), fill = Source)) + 
  geom_boxplot() + 
  scale_fill_manual(values = c("#3c0d94","#eba21a")) + 
...
## The bottom 10% have not been filtered out for this analysis to capitalize on rare species estimates ##

M_bactf <- as.data.frame(M)
M_bactf 
M_bactf2 <- rownames_to_column(M_bactf, "SAMPLE")
M_bactf2 <- M_bactf2 %>% mutate(Source = ifelse(grepl("HUM",SAMPLE),"Human","Lemur"), .after = SAMPLE)
M_bactf3 <- gather(M_bactf2, "SPECIES", "COUNT",3:length(M_bactf2))
M_bactf4 <- M_bactf3 %>% dplyr::select(Source, SPECIES, COUNT) %>% group_by(Source, SPECIES) %>% summarise(SUM = sum(COUNT))
M_bactf5 <- spread(M_bactf4, Source, SUM)

## shared species ##
table_species_overlap <- M_bactf5 %>% filter(Human > 0 & Lemur > 0)

## among those that were detected in the >0.01% and among 10% of samples ##

table_species_overlap2 <- table_species_overlap %>% filter(table_species_overlap$SPECIES %in% Total_Species_Filtered_list_bact) 

## table of species correct ##
M_bactf6 <- M_bactf5 %>% filter(M_bactf5$SPECIES %in% Total_Species_Filtered_list_bact) 
###

library(iNEXT)

ChaoRichness(M_bactf5[,c("Human","Lemur")])

i.out <- iNEXT(as.data.frame(M_bactf5[,c("Human","Lemur")]), q=0, datatype="abundance")

#i.out <- iNEXT(M_frame5[,c("Human","Lemur")], q=0, datatype="abundance")


library(ggplot2)
chao_rare_species_bact <- ggiNEXT(i.out,) + 
  scale_color_manual(values = c("#3c0d94","#eba21a")) + 
...
#Calculates distance and transforms into centered log-ratio
dist_mat_species_clr = vegdist(spec_mat, method = "aitchison", binary = FALSE, pseudocount = 1 )
cmd_res_spec_clr = cmdscale(dist_mat_species_clr, 
                   k = (nrow(spec_mat_clr) - 1),
                   eig = TRUE)

str(cmd_res_spec_clr)

#column binding the metadata to the plot
## Use Humans and Lemurs 3

metadata2 <- humans_and_lemurs2 %>% dplyr::select(Sample_ID, anon_vill,Sample_Type2) %>% filter(Sample_ID %in% spec_data_t2_bact$SAMPLE_ID) %>% arrange(Sample_ID)

## for the compositional figure: 

#### Biplot for PCA ####

prcomp_species_clr <- prcomp(dist_mat_species_clr, center = TRUE, scale. = TRUE)

##has to take an object of class "princomp"
species_biplot <- ggbiplot::ggbiplot(prcomp_species_clr, groups = metadata2$anon_vill,
         labels = NULL,
         axis.title = "PC",
         var.factor = 1.4,
  ...
  labs(fill = "Source",color = "Source") +
   scale_color_manual(values = c("#eba21a","#110063","#02000E","#7C7B7F","#93DFFD","#3c0d94","#0083B6","#1FF8FF","#007377")) + 
  scale_fill_manual(values = c("#eba21a","#110063","#02000E","#7C7B7F","#93DFFD","#3c0d94","#0083B6","#1FF8FF","#007377")) + 
  theme(panel.background = element_blank(), legend.position = 'right', panel.grid.major = element_line(colour = "grey89"),
...
Fig2A_bact
Fig2C <- fig2C + theme(plot.margin = unit(c(0.1,0.9,0.1,0.9), "inches"))
Fig2B <- species_biplot #p_vill_spec_clr
Fig2D <- chao_rare_species_bact + theme(plot.margin = unit(c(0.1,0.5,0.1,0.5), "inches"))

##Note to the author to adjust the Fig1A
Fig2 <- cowplot::plot_grid(Fig1A_bact,Fig2B,Fig2C,Fig2D, nrow = 2, ncol = 2,rel_widths = c(0.9,0.75,0.9,0), rel_heights = c(0.75,0.5,0.9,0), labels = c("a","b","c","d")) + theme(plot.background = element_rect(fill = "white", color = "white"))
...
ggsave("Fig2_dpi_300_resubmission_biplot_june.png", Fig2, height = 5.333, width = 10, dpi = 300)

#ggsave("Fig2_dpi_1200_resubmission_biplot.pdf", Fig1_bact, height = 8, width = 15, dpi = 1200)
```


## Figure 3 - Antibiotic Resistance Diversity
...
#### ggbiplot ##update this because 2 lemurs have zeros ###

prcomp_arg <- prcomp(dist_mat_amr_clr, center = TRUE, scale. = TRUE)

##has to take an object of class "princomp"
arg_biplot <- ggbiplot::ggbiplot(prcomp_arg, groups = metadata2[metadata2$Sample_ID %in% rownames(amr_gene_mat2),]$anon_vill,
         labels = NULL,
         var.factor = 1.4,
         ellipse = TRUE, ellipse.level = 0.5, ellipse.alpha = 0.1,
         circle = FALSE, 
...
### Figure 3 (Resubmission)
```{r}

RPKM_class_sum <- RPKM_class_resub %>% group_by(SAMPLE_ID, Class, Source) %>% summarise(sumTemplate = sum(template_length_kb), sumReads = sum(readCount), number_rooted_reads, RPKM = ((sum(readCount)/(sum(template_length_kb)*number_rooted_reads))*1000000000)) %>% unique()

RPKM_percent <- RPKM_class_sum %>% ungroup() %>% 
  dplyr::select(SAMPLE_ID, Class, RPKM) %>% 
  spread(Class, RPKM) %>%
  replace(is.na(.), 0) 
RPKM_percent$Class_denom <- rowSums(RPKM_percent[,2:16])

RPKM_percent_t <- RPKM_percent %>% gather(Class,"RPKM", 2:16) %>% mutate(Class_percent = (RPKM/Class_denom)*100) %>% mutate(Source = case_when(grepl("HUM",SAMPLE_ID) ~ "Human", grepl("LMR", SAMPLE_ID) ~ "Lemur")) %>%
...
Fig3A_resub <- RPKM_percent_t %>% ggplot(aes(x = SAMPLE_ID, y = Class_percent, fill = reorder(class_grouped, -Class_percent))) + geom_bar(stat = "identity") +
  scale_fill_brewer(palette = "Paired") + 
  labs(x = "Sample ") + 
  labs(x = "Sample", y = "Relative Abundance", fill = "Antibiotic class") + 
  facet_grid(. ~ Source,  scales = "free", space = "free") +  
   theme(panel.background = element_blank(), 
        axis.text.x = element_blank(),
        axis.title.x = element_blank(),
...
Fig3B_resub <- ggplot(AMR_SI, aes(x = Source, y = as.numeric(Shannon), fill = Source)) + 
  geom_boxplot() + 
  scale_fill_manual(values = c("#3c0d94","#eba21a")) + 
  labs(x = "Source", y = "Shannon Index") + 
...
Fig3D_resub <- Chao_rare_AMR + theme(plot.margin = unit(c(0.1,0.5,0.1,0.5), "inches"))
arg_biplot 
Fig3B_resub <- Fig3B_resub + theme(plot.margin = unit(c(0.2,1.4,0.05,1.4), "inches"))

Fig3_resub <- cowplot::plot_grid(Fig3A_resub,arg_biplot,Fig3B_resub,Fig3D_resub, nrow = 2, ncol = 2,rel_widths = c(0.9,0.75,0.9,0), rel_heights = c(0.75,0.5,0.9,0), labels = c("a","b","c","d")) + theme(plot.background = element_rect(fill = "white", color = "white"))

Fig3_resub

ggsave("Fig3_dpi_300_resub_biplot.png", Fig3_resub, height = 5.3333, width = 10, dpi = 300)

```

## Figure 4
See Aldex 2 Code for Figure 4
...
### Other more stable function for extracting the legend:
...
Fig5_resub <- plot_grid(pid1, shift_legend3(pid2), pidflank1, shift_legend3(pidflank2), labels="auto")

ggsave("Fig5_dpi_300.pdf", Fig5_resub, height = 5.3333, width = 10, dpi = 300)

ggsave("Fig5_dpi_300.png", Fig5_resub, height = 7, width = 10, dpi = 300)
...
dfrA1$Coverage <- as.numeric(dfrA1$Coverage)


#create a variable to indicate directionality related to strand
dfrA1$direction <- ifelse(dfrA1$Strand == "-", 0, 1)

#renaming blanks to fill in functional gaps for value in variable Gene
dfrA1$Genes <- case_when(grepl("aminoglycoside riboswitch",dfrA1$Product) ~ "attI riboswitch",
grepl("Transcriptional regulator",dfrA1$Product) ~ "transcript regulator",
grepl("Streptomycin 3''-O-adenylyltransferase",dfrA1$Product) & dfrA1$Gene == "" ~ "Streptomycin adenyltransferase gene",
grepl("hypothetical|Uncharacterized|DUF1240 domain-containing protein|MLTR-LBD domain-containing protein",dfrA1$Product) ~ "uncharacterized ORF",
grepl("UspA",dfrA1$Product) ~ "UspA gene", 
grepl("transposase",dfrA1$Product) ~ "transposase gene",
grepl("Aerotaxis", dfrA1$Product) ~ "chemotaxis gene",
grepl("regulatory region of the repBA gene",dfrA1$Product) ~ "repBA regulator",
grepl("N-acetyltransferase",dfrA1$Product) & dfrA1$Gene == "" ~ "Other acetyltransferase gene",
grepl("Replication protein C",dfrA1$Product) ~ "replication gene",
grepl("DUF2523 domain-containing protein",dfrA1$Product) ~ "Exosortase gene", 
grepl("Transposase",dfrA1$Product) ~ "transposase gene",
grepl("Diguanylate cyclase",dfrA1$Product) ~ "Diguanylate cyclase gene",
grepl("Replication initiation",dfrA1$Product) ~ "Replication initiation gene",
grepl("EAL domain-containing protein",dfrA1$Product) ~ "EAL domain gene", 
TRUE ~ dfrA1$Gene)

#dfrA1$Genes <- ifelse(dfrA1$Gene == "", "ORF", dfrA1$Gene)

## FACTORING TO MAKE NA LAST ON THE LIST ##
#dfrA1$Genes <- factor(dfrA1$Gene, levels = c("aadA","aadA1","aph(3'')-Ib" ,"aph(6)-Id","dfrA1","estX","intI1","istB",     "qacEdelta1","repA","spoIVCA","sul1","sul2","sulP","tniA","tniB","tnp","tnpA","tnpM","traX","xerD","yhbS","yacA","Unknown ORF"))

dfrA1$Genes <- factor(dfrA1$Genes, levels = c("aadA","aadA1","aph(3'')-Ib","aph(6)-Id","chemotaxis gene","dfrA1","Diguanylate cyclase gene","EAL domain gene","estX","Exosortase gene","intI1","istB","Other acetyltransferase gene","qacEdelta1", "repA", "repBA regulator","replication gene","Replication initiation gene","attI riboswitch","spoIVCA","Streptomycin adenyltransferase gene","sul1","sul2","sulP","tniA","tniB","tnpM","transcript regulator","transposase gene","traX","UspA gene","xerD","yacA","yhbS","uncharacterized ORF" ))

## Creating a color scheme that can help understand the genes present on more than one contig without having a complex legend
shared_contig_genes <- dfrA1 %>% dplyr::select(Contig_Name, Gene, Genes) %>% unique()
table(shared_contig_genes$Genes)

dfrA1$genehighlights <- case_when(dfrA1$Genes %in% c("yhbS","tnpM","tniB","tniA","sulP","sul2","sul1","intI1","estX","dfrA1","aadA1","aadA","attI riboswitch","UspA gene","xerD","qacEedelta1","aph(6)-Id","aph(3'')-Ib") ~ dfrA1$Genes, TRUE ~ "Other")

dfrA1$genehighlights <- factor(dfrA1$genehighlights, levels = c("yhbS","tnpM","tniB","tniA","sulP","sul2","sul1","intI1","estX","dfrA1","aadA1","aadA","Streptomycin adenyltransferase gene","riboswitch gene","UspA gene","xerD","qacEedelta1","aph(6)-Id","aph(3'')-Ib", "Other"))

dfrA1$Sample_ID3 <- paste(dfrA1$Sample_ID2,dfrA1$vill2,sep="\n")

library(pals)
##GET MORE COLORS FOR THE NUMBER OF GENES
colourCount = length(unique(dfrA1$genehighlights))
#getPalette = colorRampPalette(brewer.pal(10, "Set3"))
palette <- polychrome(n=colourCount)

## DUMMY DATA TO ALIGN TO A SPECIFIC GENE ##
dummies_dfrA1 <- make_alignment_dummies(
  dfrA1,
  aes(xmin =Start, xmax = Stop, y = Sample_ID3, id = genehighlights),
  on = "dfrA1",side = "right"
)
...
## FINAL PLOT ##
dfrA1_plot <- ggplot(dfrA1[dfrA1$Type %in% c("cds","sorf"),], aes(xmin = Start, xmax = Stop, y = Sample_ID3, forward = direction, fill = genehighlights, label = genehighlights)) +
    geom_feature(
    data =dfrA1[!(dfrA1$Type %in% c("cds","sorf")),] ,
    aes(x = Start, y = Sample_ID3)
  ) +
  geom_feature_label(
    data = dfrA1[!(dfrA1$Type %in% c("cds","sorf")),],
    aes(x = Start, y = Sample_ID3, label = Genes), size = 14
  ) + 
  geom_gene_arrow() +
  geom_gene_label(align = "left") +
  ...
  theme_genes() +
  labs( y = "Sample", fill = "Genes") + 
  theme(legend.position = "bottom")

dfrA1_plot

ggsave("Fig6_dpi_300_v2.pdf", dfrA1_plot, height = 8, width = 14, dpi = 300)


ggsave("Fig6_dpi_300_v2.png", dfrA1_plot, height = 7, width = 10, dpi = 300)
